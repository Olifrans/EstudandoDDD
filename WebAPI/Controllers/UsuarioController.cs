...
namespace WebAPI.Controllers
{
    ...
    public class UsuarioController : ControllerBase
    {
        ...
        public async Task<IActionResult> CriarToken([FromBody]Login login)
        {
            ...
            if (resultado)
            {
                var token = new TokenJWTBuilder()
                     ...
                     //Configarações personalizadas ou da sua empresa
                 .AddSubject("Olifrans - Estudando Domain-Driven Design (Desenvolvimento dirigido ao domínio – DDD) ")
                 .AddIssuer("Teste.Securiry.Bearer")
                 .AddAudience("Teste.Securiry.Bearer")
                 .AddClaim("idUsuario", "1")
                 .AddExpiry(5)
                 .Builder();
        ...
        {
            //Validalções de login e senha de usuario
            if (string.IsNullOrWhiteSpace(login.email) || string.IsNullOrWhiteSpace(login.senha))
                return Ok("Falta preenche os dados");
        ...
        public async Task<IActionResult> CriarTokenIdentity([FromBody] Login login)  //Metodo criado na forma do Identity
        {
            //validalções de usuario
            if (string.IsNullOrWhiteSpace(login.email) || string.IsNullOrWhiteSpace(login.senha))
                return Unauthorized();
        ...
        public async Task<IActionResult> AdicionaUsuarioIdentity([FromBody] Login login)  //Metodo criado na forma do Identity
        {
            if (string.IsNullOrWhiteSpace(login.email) || string.IsNullOrWhiteSpace(login.senha))
